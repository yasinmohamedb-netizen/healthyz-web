...
export default function CheckoutScreen() {
  ...
  // Listen for address update callback (from address page)
  useEffect(() => {
  ...
  const updatePriceFromBackend = async (promoInput = promoCode) => {
    try {
      const resp = await axios.post(`${BASE_URL}/checkout-price/calculate`, {
      ...
      console.log("PRICE CALCULATION ERROR:", err.response?.data);
    }
  };
  ...
  // ================================
  // PROMO INPUT CHANGE — REMOVE DISCOUNT INSTANTLY
  // ================================
  const handlePromoChange = (value) => {
    setPromoCode(value);

    // Reset promo immediately (same as React Native)
    setDiscount(0);
    setPromoMessage("");

    // Call backend WITHOUT promo to refresh totals
    updatePriceFromBackend("");
  };

  // APPLY PROMO — Only when clicking Apply
  const applyPromo = () => {
...
<div className="summary-line tax-inclusive">
  <span>Inclusive of all taxes</span>
          ...
          <p className="secure-text">🔒 Secure Checkout – Your payment details are safe</p>
        </div>
      </div>
    </div>
  );
